...
$link = connectToDB();
  ...
  $FCnirp = new FusionCharts("Pie3D","450","450");

  # Set Relative Path of chart swf file.
  $FCnirp->setSwfPath("../../FusionCharts/");

  //Store chart attributes in a variable for ease of use
  $strParam="caption=NIR Progress Output report;subCaption=By Sample priority;pieSliceDepth=30; showBorder=1;showNames=1;formatNumberScale=0;numberSuffix= ;decimalPrecision=0";

  # Set chart attributes
  $FCnirp->setChartParams($strParam);
  
  // Fetch all factory records using SQL Query
  // Store chart data values in 'total' column/field
  // and category names in 'FactoryName'
  $strQuery = "select nir.status, nir.priority,count(nir.status) as cs from tblnirtaskactivity nir  group by nir.priority";
  ...
  if ($result)
  {
    $FCnirp->addDataFromDatabase($result, "cs", "priority");
  }
