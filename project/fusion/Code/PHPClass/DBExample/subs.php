...
$link = connectToDB();
  ...
  $FCsubs = new FusionCharts("Pie3D","450","450");

  # Set Relative Path of chart swf file.
  $FCsubs->setSwfPath("../../FusionCharts/");

  //Store chart attributes in a variable for ease of use
  $strParam="caption=Subsampling Progress Output report;subCaption=By Sample status;pieSliceDepth=30; showBorder=1;showNames=1;formatNumberScale=0;numberSuffix= ;decimalPrecision=0";

  # Set chart attributes
  $FCsubs->setChartParams($strParam);
  
  // Fetch all factory records using SQL Query
  // Store chart data values in 'total' column/field
  // and category names in 'FactoryName'
  $strQuery = "select sub.priority, sub.status, count(sub.status) as cs from tblsubsampletaskactivity sub  group by sub.status";
  ...
  if ($result)
  {
    $FCsubs->addDataFromDatabase($result, "cs", "status");
  }
