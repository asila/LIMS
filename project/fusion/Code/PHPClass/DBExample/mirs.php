...
$link = connectToDB();
  ...
  $FCmirs = new FusionCharts("Pie3D","450","450");

  # Set Relative Path of chart swf file.
  $FCmirs->setSwfPath("../../FusionCharts/");

  //Store chart attributes in a variable for ease of use
  $strParam="caption=MIR Progress Output report;subCaption=By Sample status;pieSliceDepth=30; showBorder=1;showNames=1;formatNumberScale=0;numberSuffix= ;decimalPrecision=0";

  # Set chart attributes
  $FCmirs->setChartParams($strParam);
  
  // Fetch all factory records using SQL Query
  // Store chart data values in 'total' column/field
  // and category names in 'FactoryName'
  $strQuery = "select mir.priority, mir.status, count(mir.status) as cs from tblmirtaskactivity mir  group by mir.status";
  ...
  if ($result)
  {
    $FCmirs->addDataFromDatabase($result, "cs", "status");
  }
