...
$link = connectToDB();
  ...
  $FCxrds = new FusionCharts("Pie3D","450","450");

  # Set Relative Path of chart swf file.
  $FCxrds->setSwfPath("../../FusionCharts/");

  //Store chart attributes in a variable for ease of use
  $strParam="caption=XRD Progress Output report;subCaption=By Sample status;pieSliceDepth=30; showBorder=1;showNames=1;formatNumberScale=0;numberSuffix= ;decimalPrecision=0";

  # Set chart attributes
  $FCxrds->setChartParams($strParam);
  
  // Fetch all factory records using SQL Query
  // Store chart data values in 'total' column/field
  // and category names in 'FactoryName'
  $strQuery = "select xrd.priority, xrd.status, count(xrd.status) as cs from tblxrdtaskactivity xrd  group by xrd.status";
  ...
  if ($result)
  {
    $FCxrds->addDataFromDatabase($result, "cs", "status");
  }
