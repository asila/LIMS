...
$link = connectToDB();
  ...
  $strParam="caption=Barcoding Progress Output report;subCaption=By Sample status;pieSliceDepth=30; showBorder=1;showNames=1;formatNumberScale=0;numberSuffix= ;decimalPrecision=0";
  ...
  $strQuery = "select bar.priority, bar.status, count(bar.status) as cs from tblbartaskactivity bar  group by bar.status";
  ...
  if ($result)
  {
    $FCbarp->addDataFromDatabase($result, "cs", "status");
  }
