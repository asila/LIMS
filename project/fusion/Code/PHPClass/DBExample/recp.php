...
$link = connectToDB();
  ...
  $FC2 = new FusionCharts("Pie3D","450","450");

  # Set Relative Path of chart swf file.
  $FC2->setSwfPath("../../FusionCharts/");

  //Store chart attributes in a variable for ease of use
  $strParam="caption=Reception Progress Output report;subCaption=By Sample priority;pieSliceDepth=30; showBorder=1;showNames=1;formatNumberScale=0;numberSuffix= ;decimalPrecision=0";
  ...
  $strQuery = "select b.priority, b.status, count(b.status) as cs from tblrectaskactivity b  group by b.priority";
  ...
  if ($result)
  {
    $FC2->addDataFromDatabase($result, "cs", "priority");
  }
