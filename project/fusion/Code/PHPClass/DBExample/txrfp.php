...
$link = connectToDB();
  ...
  $FCtxrfp = new FusionCharts("Pie3D","450","450");

  # Set Relative Path of chart swf file.
  $FCtxrfp->setSwfPath("../../FusionCharts/");

  //Store chart attributes in a variable for ease of use
  $strParam="caption=TXRF Progress Output report;subCaption=By Sample priority;pieSliceDepth=30; showBorder=1;showNames=1;formatNumberScale=0;numberSuffix= ;decimalPrecision=0";

  # Set chart attributes
  $FCtxrfp->setChartParams($strParam);
  
  // Fetch all factory records using SQL Query
  // Store chart data values in 'total' column/field
  // and category names in 'FactoryName'
  $strQuery = "select txrf.priority, txrf.status, count(txrf.status) as cs from tbltxrftaskactivity txrf  group by txrf.priority";
  ...
  if ($result)
  {
    $FCtxrfp->addDataFromDatabase($result, "cs", "priority");
  }
