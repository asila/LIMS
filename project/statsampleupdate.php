...
<table class="responsive" style="width:100%;">
	<tr align="center">
		<td>
       <a href="viewStatsampleSchedule.php?id=<?php echo $id ?>"><img SRC="image/back.png" ALT="back" width="40" height="40" ></img></a>
		</td>
	</tr>
...
$newdate = date('Y-m-d', strtotime($taskdate. " + $taskestdate days"));
			  $id =$_REQUEST['id']; 
$query = "UPDATE $dbtable SET priority='{$statusp}',status='{$statuslev}',assignto='{$anadep}',taskcreatedon='{$taskdate}',estimation='{$taskestdate}',actualtime='{$newdate}',statcom='{$taskcom}' WHERE taskid LIKE '%$id%'";
	//Execute query
...
$col++;
		...
		echo '<option  value="'.$rows3['analysis'].'">'.$rows3['analysis'].'</option>';
						...
						echo ' </td><td> ';
						?>
						<input type="text" size="20" id="taskdate" name="taskdate"  value="<?php echo $rows['taskcreatedon'];?>" /></a>
						<?php
						echo ' </td><td> ';
						?>
